...
class Client {
  ...
  constructor(options) {
    ...
    try {
      ...
      switch (err) {
        case 'pathname':
          throw new TypeError('baseUrl option must not include a pathname.', { cause: err });
        ...
        case 'username':
          throw new TypeError('baseUrl option must not include username.', { cause: err });

        case 'password':
          throw new TypeError('baseUrl option must not include password.', { cause: err });

        case 'hash':
          throw new TypeError('baseUrl option must not include hash.', { cause: err });

        default:
          throw new TypeError('Invalid baseUrl option provided.', { cause: err });
      }
    }
    ...
    request.on('response', function (response) {
      ...
      response.on('end', function () {
        ...
        if (statusCode < 200 || statusCode > 399) {
          ...
          // @ts-expect-error unknown shape of parsedData
          const errDetails = (Array.isArray(parsedData?.errors) ? parsedData.errors : [])
            .map((/** @type {any} */ err) =>
              [err?.code, err?.detail ?? err?.title].filter((v) => v != null).join(' '),
            )
            .filter(Boolean);
          const errSuffix = errDetails.length ? ` (${errDetails.join('; ')})` : '';
          return callback(
            new Error(`API error: ${response.statusCode} ${errString}${errSuffix}`),
            null,
            response,
          );
        }
   ...
   *
   * @see [API Docs](https://www.tumblr.com/docs/en/api/v2#posts---createreblog-a-post-neue-post-format)
   *
   * @example
   ...
   * Edit an NPF post
   *
   * @see [API Docs](https://www.tumblr.com/docs/en/api/v2#postspost-id---editing-a-post-neue-post-format)
   *
   * @param  {string} blogIdentifier - blog name or URL
   ...
   *
   * @see [API Docs](https://www.tumblr.com/docs/api/v2#posting)
   *
   * @param  {string} blogIdentifier - blog name or URL
   * @param  {Record<string,any>} params
   * @param  {TumblrClientCallback} [callback] **Deprecated** Omit the callback and use the promise form
