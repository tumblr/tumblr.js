...
function getRequest(requestGet, credentials, baseUrl, apiPath, requestOptions, params, callback) {
  ...
  return requestGet(
    extend(
      {
        url: url.toString(),
        oauth: credentials.token ? credentials : undefined,
        json: true,
      },
      requestOptions
    ),
...
class TumblrClient {
  ...
  constructor(options) {
    ...
    if (
      ...
      for (const propertyName of OAUTH_CREDENTIAL_OPTIONAL_PROPERTIES) {
        if (!options[propertyName] || typeof options[propertyName] !== 'string') {
          throw new TypeError(
            `Provide consumer_key or all oauth credentials. Invalid ${propertyName} provided.`
          );
        }
      }

      /**
       * @type {{readonly consumer_key: string} | {readonly consumer_key:string; readonly consumer_secret:string; readonly token:string; readonly token_secret:string}}
       */
      this.credentials = {
