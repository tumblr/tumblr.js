...
function getRequest(requestGet, credentials, baseUrl, apiPath, requestOptions, params, callback) {
  ...
  // Use the `api_key` query param if we don't have full credentials
  /** @type {TumblrClient['credentials'] | undefined} */
  let oauth;
  if (!credentials.token) {
    url.searchParams.set('api_key', credentials.consumer_key);
  } else {
    oauth = credentials;
  }
...
/**
 * @typedef Options
 * @property {string} [baseUrl] The API url, defaults to "https://api.tumblr.com"
...
class TumblrClient {
  ...
  constructor(options) {
    ...
    // A consumer_key is required. It can be used as api_key query param by itself
    if (!options.consumer_key || typeof options.consumer_key !== 'string') {
      throw new TypeError('You must provide a consumer_key.');
    }

    // All or none of these must be provided
    const OAUTH_CREDENTIAL_OPTIONAL_PROPERTIES = /** @type {const} */ ([
      'consumer_secret',
      'token_secret',
      'token',
    ]);
    ...
    if (
      OAUTH_CREDENTIAL_OPTIONAL_PROPERTIES.some((propertyName) =>
      ...
      /**
       * @type {{readonly consumer_key: string; readonly consumer_secret?: string; readonly token?: string; readonly token_secret?: string}}
       */
      this.credentials = {
      ...
      this.credentials = { consumer_key: options.consumer_key };
    }
  ...
  addMethod(methodName, apiPath, paramNames, requestType) {
    ...
    set(this, methodName, createFunction(methodName, namedParams, methodBody));
  }
}
