...
var calls = {
  ...
  postCreation: function (type, requireOptions, acceptsData) {
    return function (blogName, options, callback) {
      ...
      options = setDefaultPostOptions(options);
      options.type = type;
      if (!acceptsData) {
        delete options.data;
      }
  ...
  postEditing: function(blogName, options, callback) { 
    options = setDefaultPostOptions(options);
    return function(blogName, options, callback) { 
  ...
  setDefaultPostOptions: function(options) {
    for (var val in postingDefaults) {
       if (postingDefaults.hasOwnProperty(val)) {
          options[val] = (options[val] === null) ? postingDefaults[val] : options[val];
       }
    }
    return options;
  },
...
TumblrClient.prototype = {
  ...
  edit: calls.postEditing(blogName, options, callback),
