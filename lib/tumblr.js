...
class TumblrClient {
  ...
  getRequest(apiPath, paramsOrCallback, callback) {
    ...
    const [url, requestData] = this.prepareRequestUrlAndRequestData(apiPath, params, 'GET');
  ...
  createPost(blogIdentifier, params, callback) {
    const data = TumblrClient.transformNpfParams(params);
   ...
   * @private
   */
  static transformNpfParams(params) {
    const transformed = {
      json: JSON.stringify({
        ...params,
