...
function requestCallback(callback) {
  ...
  return function (err, response, body) {
    ...
    if (response.statusCode >= 400) {
      ...
      return callback(new Error('API error: ' + response.statusCode + ' ' + errString), body, response);
    }
    if (body && body.response) {
      return callback(null, body.response, response);
    } else {
      return callback(new Error('API error (malformed API response): ' + body), body, response);
    }
  };
}
