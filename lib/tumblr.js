 ...
 *
 * @param  {TumblrClientCallback} callback - function to wrap
 *
 * @return {TumblrClientCallback} request callback
 *
 * @private
 */
function makeCallback(callback) {
  return (
    ...
    function (res) {
      // If we don't have a callback, consume response to free memory
      if (!callback) {
        res.resume();
        return;
      }
      ...
      res.on('end', () => {
        try {
          ...
          if (/** @type {number} */ (res.statusCode) >= 400) {
            const errString = parsedData.meta ? parsedData.meta.msg : parsedData.error;
            return callback(new Error('API error: ' + res.statusCode + ' ' + errString), null, res);
          }
 ...
 * Make a get request
 *
 * @param  {null|oauth.OAuth} oauth - function that performs a get request
 * @param  {TumblrClient['credentials']} credentials - OAuth credentials
 ...
 *
 * @return {http.ClientRequest} Request object
 *
 * @private
 */
function getRequest(oauth, credentials, baseUrl, apiPath, requestOptions, params, callback) {
  params = params || {};
  ...
  if (credentials.auth === 'apiKey') {
    url.searchParams.set('api_key', credentials.apiKey);
  }

  /** @type {http.ClientRequest} */
  const request = makeRequest(url, 'GET', callback);

  if (oauth && credentials.auth === 'oauth1') {
  ...
  request.on('response', function (response) {
    ...
    response.on('end', function () {
      ...
      try {
        ...
        callback(new Error(`API error (malformed API response): ${data}`), null, response);
        return;
      }
 ...
 * @private
 */
function postRequest(oauth, credentials, baseUrl, apiPath, requestOptions, params, callback) {
  params = params || {};
  ...
  /** @type {http.ClientRequest} */
  let request;
  if (credentials.auth === 'oauth1' && oauth) {
    request = oauth.post(
      url.toString(),
      credentials.token,
      credentials.token_secret,
    ...
    const httpModel = url.protocol === 'http' ? http : https;
    request = httpModel.request(url, { method: 'POST' }, function (err, response, body) {
      try {
      ...
      makeCallback(callback)(err, response, body);
    });
  }
  ...
  const dataKeys = ['data'];
  request.form(omit(params, dataKeys));
...
class TumblrClient {
  ...
  constructor(options) {
    ...
    this.requestOptions = /** @type {const} */ ({
      followRedirect: false,
      headers: {
        'User-Agent': 'tumblr.js/' + this.version,
      },
    });
